...
- 🖼️ **图片上传支持**：点击上传、粘贴截图、拖拽图片，支持 Base64 内嵌和 GitHub 图床
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **Base64 内嵌** | 图片转为 Base64 直接嵌入 Markdown | 零配置，适合小图、临时使用 |
| **GitHub 图床** | 上传到 GitHub 仓库，获取 CDN 链接 | 需配置 Token，适合正式文章 |

> **配置 GitHub 图床步骤**：
> 1. 在 [GitHub Settings > Developer settings > Personal access tokens](https://github.com/settings/tokens) 生成 Token（需勾选 `repo` 权限）
> 2. 准备一个公开仓库作为图床，例如 `cscsxx606/images`
> 3. 点击顶部「配置」按钮填入 Token 和仓库名
